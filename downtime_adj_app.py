...
def process_batch_data(batch_data, downtime_data):
    ...
    try:
        ...
        if downtime_df is None or downtime_df.empty or 'date' not in downtime_df.columns or 'downtime_pct' not in downtime_df.columns:
            st.error("Downtime data must include non-empty 'date' and 'downtime_pct' columns.")
            return None, None, None
...
def copy_to_clipboard(data_type='rates', all_wells=False):
    ...
    if all_wells and st.session_state.processed_wells:
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            ...
            for well_name, well_data in st.session_state.processed_wells.items():
                ...
                # Rates sheet
                rate_df = df[['adj_oil_rate', 'adj_gas_rate', 'adj_water_rate', 'adj_liquid_rate']].copy()
                rate_df.columns = ['Oil Rate', 'Gas Rate', 'Water Rate', 'Liquid Rate']
                
                # Volumes sheet
                volume_df = df[['adj_oil_volume', 'adj_gas_volume', 'adj_water_volume', 'adj_liquid_volume']].copy()
                volume_df.columns = ['Oil Volume', 'Gas Volume', 'Water Volume', 'Liquid Volume']
                
                # Write to Excel
                rate_df.to_excel(writer, sheet_name=f"{well_name[:15]}_Rates")
                volume_df.to_excel(writer, sheet_name=f"{well_name[:15]}_Volumes")
    ...
    if data_type == 'rates':
        ...
        st.text_area("Copyable TSV Data", tsv_data, height=150, label_visibility="collapsed")
...
def generate_selected_wells_excel():
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl', date_format='YYYY-MM-DD') as writer:
        for well in selected_export_wells:
            ...
            export_df.to_excel(writer, sheet_name=f"{well[:15]}", index=False, startrow=4)
    buffer.seek(0)
    ...
    for well in selected_export_wells:
        ...
        for row in ws.iter_rows(min_row=6, min_col=1, max_col=1, max_row=ws.max_row):
            for cell in row:
...
with col1:
    ...
    well_name = st.text_input("Well Name", placeholder="Enter well name...(leave blank if batch processing)", key="well_name_input", label_visibility="collapsed")
    ...
    if well_name and well_name != st.session_state.well_name:
        ...
        if 'forecast_data' in st.session_state and st.session_state.forecast_data:
            ...
            st.session_state.forecast_data = forecast_df.to_csv(sep='\t', index=False)
    forecast_columns = ['Well Name', 'date', 'oil_rate', 'gas_rate', 'water_rate']
...
with col2:
    ...
    scenario_name = st.text_input("Downtime Scenario", placeholder="Enter downtime scenario (i.e., OL3)...(leave blank if loading from file)", key="scenario_input", label_visibility="collapsed")
    ...
    if scenario_name and scenario_name != st.session_state.scenario_name:
        ...
        if 'downtime_data' in st.session_state and st.session_state.downtime_data:
            ...
            st.session_state.downtime_data = downtime_df.to_csv(sep='\t', index=False)
    downtime_columns = ['Scenario', 'date', 'downtime_pct']
    # Always use the latest session state for table display
    if 'downtime_data' in st.session_state and st.session_state.downtime_data:
        try:
...
with colA:
    st.button("Process Data", type="primary", key="process_single", on_click=handle_process_data)
with colB:
    def clear_input():
        st.session_state.forecast_data = ""
        st.session_state.downtime_data = ""
...
def process_and_store_batch_data():
    ...
    try:
        # Process batch data
        well_data, downtime_df, unique_wells = process_batch_data(st.session_state.batch_data, st.session_state.downtime_data)
        ...
        for i, well_name in enumerate(unique_wells):
            ...
            if errors:
                st.warning(f"Skipping {well_name} due to errors: {'; '.join(errors)}")
                continue
...
if st.session_state.processed_data is not None and 'processed_wells' in st.session_state and st.session_state.processed_wells:
    ...
    st.session_state.figure = create_plotly_figure(st.session_state.processed_data, well_name=selected_well_name)
    ...
    if selected_well_name and selected_well_name in st.session_state.processed_wells:
        ...
        df['boe_input'] = df['oil_rate'] + df['gas_rate'] / 6 + df['water_rate']
        df['boe_processed'] = df['adj_oil_rate'] + df['adj_gas_rate'] / 6 + df['adj_water_rate']
        df['oil_volume_processed'] = df['adj_oil_rate'] * df['daysInMonth'] / 1000
        df['gas_volume_processed'] = df['adj_gas_rate'] * df['daysInMonth'] / 1000
        df['water_volume_processed'] = df['adj_water_rate'] * df['daysInMonth'] / 1000
    ...
    display_cols = [
        'Year',
        'Avg Oil Rate', 'Avg Gas Rate', 'Avg Water Rate', 'Avg BOE/d',
        'BOE Vol, MBOE',
        'Avg Downtime',
        'Avg Oil Rate (DT)', 'Avg Gas Rate (DT)', 'Avg Water Rate (DT)', 'Avg BOE/d (DT)',
        'BOE Vol, MBOE (DT)'
    ]
    ...
    summary_df = summary_df[summary_df['Year'].apply(lambda x: isinstance(x, (int, np.integer)))]
    st.markdown(f'<p class="table-title">Well-Year Summary Table ({selected_well_name})</p>', unsafe_allow_html=True)
    st.dataframe(summary_df, use_container_width=True, height=40 * len(summary_df) + 40, hide_index=True)
